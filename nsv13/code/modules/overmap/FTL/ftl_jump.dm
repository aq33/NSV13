...
/obj/structure/overmap/proc/jump_end(datum/star_system/target_system, misjump = FALSE)
	...
	SEND_SIGNAL(src, COMSIG_FTL_STATE_CHANGE)
	if(!misjump)
		relay(ftl_drive.ftl_exit, "<span class='warning'>Czujesz, jak statek się zatrzymuje.</span>", loop=FALSE, channel = CHANNEL_SHIP_ALERT)
	else
		...
		relay(message = "<span class='warning'>Czujesz, jak statek gwałtownie opuszcza nadprzestrzeń!</span>") // AQ: Nie wiem jak ładniej przetłumaczyć "drive space".
