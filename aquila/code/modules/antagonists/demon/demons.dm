...
/datum/antagonist/sinfuldemon
	...
	//job_rank = ROLE_SINFULDEMON Nie wiem po co to
	show_to_ghosts = TRUE
...
/datum/antagonist/sinfuldemon/proc/demon_burn() //sinful demons are even more vulnerable to the chapel than vampires, but can turn into their true form to negate this.
	...
	if(L.stat != DEAD) //demons however wont dust from the chapel so this needs to be a check to avoid spam while they're already dead
		if(prob(50) && L.health >= 50)
			switch(L.health)
				if(85 to 100)
					L.visible_message("span class='warning'[L]'s skin begins to heat up and darken!</span>", \
					"<span class='danger'Your flesh begins to sear...</span>")
				if(60 to 85)
					L.visible_message("<span class='warning'[L]'s skin begins to melt apart!</span>", \
					"<span class='danger'Your skin is melting!", \
					"<span class='danger'You hear sizzling.</span>")
			L.adjustFireLoss(5)
		else if(L.health < 60)
			if(!L.on_fire)
				L.visible_message("<span class='warning'[L] lights up in a holy blaze!</span>", \
				"<span class='danger'Your skin catches fire!</span>")
				L.emote("scream")
			else
				L.visible_message("<span class='warning'[L] continues to burn!</span>", \
				"<span class='danger'You continue to burn!</span>")
...
/datum/antagonist/sinfuldemon/proc/forge_objectives()
	...
	switch(demonsin)//the 5 most interesting of the 8 sins. Left out sloth because it sounds boring, couldn't think of a good enough objective/power for acedia, and lust for obvious reasons.
		...
		if(SIN_WRATH)
			...
			if(prob(50))
				var/N = pick(/datum/objective/assassinate)
...
/datum/antagonist/sinfuldemon/greet()
	...
	to_chat(owner.current, "["<span class='warning'Do your best to complete your objectives without unnessecary death, unless you are a wrathful demon.</span>"]<br>")
