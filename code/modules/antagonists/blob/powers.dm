		...
		for(var/obj/O in T)
			if(istype(O, /obj/structure/blob))
				if(istype(O, /obj/structure/blob/normal))
					...
					to_chat(src, "<span class='warning'>Jest tu już grzyb!</span>")
					return 0
			else if(O.density)
				to_chat(src, "<span class='warning'>W tym miejscu jest zbyt gęsto by postawić tutaj twój rdzeń!</span>")
				return 0
...
/mob/camera/blob/verb/create_blobbernaut()
	...
	var/list/mob/dead/observer/candidates = pollGhostCandidates("Czy chesz zagrać jako Grzybonauta odmiany [blobstrain.name]?", ROLE_BLOB, /datum/role_preference/midround_ghost/blob, 7.5 SECONDS, ignore_category = POLL_IGNORE_BLOB_HELPER) //players must answer rapidly
	if(LAZYLEN(candidates)) //if we got at least one candidate, they're a blobbernaut now.
		...
		to_chat(blobber, "<b><font color=\"[blobstrain.color]\">[blobstrain.name]</b></font> Jako [blobstrain.shortdesc ? "[blobstrain.shortdesc]" : "[blobstrain.description]"]")
	else
